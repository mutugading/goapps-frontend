// POST /api/v1/iam/employee-levels/[employeeLevelId]/submit
...
export async function POST(request: NextRequest, context: RouteContext) {
    try {
        ...
        const response = await client.submitEmployeeLevel(
        ...
        console.error("Error submitting employee level:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to submit employee level",
                    validationErrors: [],
                },
            },
            { status: 500 }
