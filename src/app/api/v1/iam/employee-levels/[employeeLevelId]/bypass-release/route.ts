// POST /api/v1/iam/employee-levels/[employeeLevelId]/bypass-release
...
export async function POST(request: NextRequest, context: RouteContext) {
    try {
        ...
        const response = await client.bypassReleaseEmployeeLevel(
        ...
        console.error("Error bypass releasing employee level:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to bypass release employee level",
                    validationErrors: [],
                },
            },
            { status: 500 }
