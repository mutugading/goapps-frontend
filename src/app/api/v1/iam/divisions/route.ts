// IAM Divisions routes - List and Create
...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await client.listDivisions(
            {
        ...
        return NextResponse.json(
            {
                base: { isSuccess: false, statusCode: "500", message: "Failed to fetch divisions", validationErrors: [] },
                data: [],
...
export async function POST(request: NextRequest) {
    try {
        ...
        const metadata = createMetadataFromRequest(request)
        const client = getDivisionClient()
        const response = await client.createDivision(body, metadata)
        return NextResponse.json({ base: response.base, data: response.data })
    } catch (error) {
        if (isGrpcError(error)) return handleGrpcError(error)
        console.error("Error creating division:", error)
        return NextResponse.json(
            { base: { isSuccess: false, statusCode: "500", message: "Failed to create division", validationErrors: [] } },
            { status: 500 }
        )
    }
}
