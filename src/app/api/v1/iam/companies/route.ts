...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json(
            {
                base: { isSuccess: false, statusCode: "500", message: "Failed to fetch companies", validationErrors: [] },
                data: [],
...
export async function POST(request: NextRequest) {
    try {
        ...
        const metadata = createMetadataFromRequest(request)
        const client = getCompanyClient()
        const response = await client.createCompany(body, metadata)
        return NextResponse.json({ base: response.base, data: response.data })
    } catch (error) {
        if (isGrpcError(error)) return handleGrpcError(error)
        console.error("Error creating company:", error)
        return NextResponse.json(
            { base: { isSuccess: false, statusCode: "500", message: "Failed to create company", validationErrors: [] } },
            { status: 500 }
        )
    }
}
