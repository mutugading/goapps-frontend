...
export async function POST(request: NextRequest, { params }: { params: Promise<{ requestId: string }> }) {
  try {
    ...
    const response = await client.decideCostProductRequestFeasibility({
    ...
    return NextResponse.json({ base: { isSuccess: false, statusCode: "500", message: "Failed to decide feasibility", validationErrors: [] } }, { status: 500 })
  }
}
