...
export async function POST(request: NextRequest, { params }: { params: Promise<{ requestId: string }> }) {
  try {
    ...
    const response = await client.linkExistingRoute(
      { requestId: Number(requestId), routeHeadId: Number(body.routeHeadId) || 0 },
      metadata,
    )
    ...
    return NextResponse.json(
      { base: { isSuccess: false, statusCode: "500", message: "Failed to link route", validationErrors: [] } },
      { status: 500 },
    )
  }
}
