// Finance Intermingling routes - List and Create

import { NextRequest, NextResponse } from "next/server"
import { getInterminglingClient, createMetadataFromRequest, isGrpcError, handleGrpcError } from "@/lib/grpc"

// GET /api/v1/finance/interminglings - List Interminglings with filters
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await client.listInterminglings(
            {
        ...
        console.error("Error fetching Interminglings:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to fetch interminglings",
                    validationErrors: [],
                },
                data: [],
...
// POST /api/v1/finance/interminglings - Create Intermingling
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await client.createIntermingling(body, metadata)
        ...
        if (isGrpcError(error)) return handleGrpcError(error)
        console.error("Error creating Intermingling:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to create intermingling",
                    validationErrors: [],
                },
            },
            { status: 500 }
