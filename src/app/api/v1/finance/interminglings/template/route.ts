// GET /api/v1/finance/interminglings/template - Download Intermingling Import Template
...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await client.downloadInterminglingTemplate({}, metadata)
        ...
        console.error("Error downloading Intermingling template:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to download intermingling template",
                    validationErrors: [],
                },
            },
            { status: 500 }
