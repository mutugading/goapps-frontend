// Finance ProductGrade routes - List and Create

import { NextRequest, NextResponse } from "next/server"
import { getProductGradeClient, createMetadataFromRequest, isGrpcError, handleGrpcError } from "@/lib/grpc"

// GET /api/v1/finance/product-grades - List Product Grades with filters
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await client.listProductGrades(
            {
        ...
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to fetch product grades",
                    validationErrors: [],
                },
                data: [],
...
// POST /api/v1/finance/product-grades - Create Product Grade
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await client.createProductGrade(body, metadata)
        ...
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to create product grade",
                    validationErrors: [],
                },
            },
            { status: 500 }
