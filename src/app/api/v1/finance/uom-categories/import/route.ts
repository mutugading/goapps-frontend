// POST /api/v1/finance/uom-categories/import - Import UOM Categories from Excel
...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await client.importUOMCategories({
        ...
        console.error("Error importing UOM Categories:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to import UOM categories",
                    validationErrors: [],
                },
                successCount: 0,
                skippedCount: 0,
