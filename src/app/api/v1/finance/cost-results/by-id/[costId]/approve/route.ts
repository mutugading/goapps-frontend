// CostCalc — approve a cost result.
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ costId: string }> }) {
  try {
    ...
    const response = await client.approveCostResult({ costId: Number(costId) }, metadata)
    return NextResponse.json({ base: response.base, data: response.result })
  } catch (error) {
    if (isGrpcError(error)) return handleGrpcError(error)
    return NextResponse.json(
      { base: { isSuccess: false, statusCode: "500", message: "Failed to approve cost result", validationErrors: [] } },
      { status: 500 },
    )
  }
}
