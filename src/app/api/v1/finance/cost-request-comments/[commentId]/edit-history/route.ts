...
export async function GET(request: NextRequest, { params }: { params: Promise<{ commentId: string }> }) {
  try {
    ...
    const response = await client.listCostCommentEditHistory({ commentId: Number(commentId) }, metadata)
    return NextResponse.json({ base: response.base, data: response.data })
  } catch (error) {
    if (isGrpcError(error)) return handleGrpcError(error)
    return NextResponse.json({ base: { isSuccess: false, statusCode: "500", message: "Failed to load edit history", validationErrors: [] }, data: [] }, { status: 500 })
  }
}
