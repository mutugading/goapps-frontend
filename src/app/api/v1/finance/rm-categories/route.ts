// Finance RM Category routes - List and Create

import { NextRequest, NextResponse } from "next/server"
import { getRmCategoryClient, createMetadataFromRequest, isGrpcError, handleGrpcError } from "@/lib/grpc"

// GET /api/v1/finance/rm-categories - List RM Categories with filters
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await client.listRMCategories(
            {
        ...
        console.error("Error fetching RM Categories:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to fetch raw material categories",
                    validationErrors: [],
                },
                data: [],
...
// POST /api/v1/finance/rm-categories - Create RM Category
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await client.createRMCategory(body, metadata)
        ...
        if (isGrpcError(error)) return handleGrpcError(error)
        console.error("Error creating RM Category:", error)
        return NextResponse.json(
            {
                base: {
                    isSuccess: false,
                    statusCode: "500",
                    message: "Failed to create raw material category",
                    validationErrors: [],
                },
            },
            { status: 500 }
