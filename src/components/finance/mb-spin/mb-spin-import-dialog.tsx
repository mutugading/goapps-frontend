...
import { useImportMBSpins, useDownloadMBSpinTemplate } from "@/hooks/finance/use-mb-spin"
import { readFileAsBytes } from "@/lib/api"

interface MBSpinImportDialogProps {
  open: boolean
...
export function MBSpinImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: MBSpinImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportMBSpins()
  const templateMutation = useDownloadMBSpinTemplate()
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import MB Spins</DialogTitle>
          <DialogDescription>
            Import MB Spins from an Excel file. Download the template first to
