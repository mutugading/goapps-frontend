...
import { useImportProductGrades, useDownloadProductGradeTemplate } from "@/hooks/finance/use-product-grade"
import { readFileAsBytes } from "@/lib/api"

interface ProductGradeImportDialogProps {
  open: boolean
...
export function ProductGradeImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: ProductGradeImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportProductGrades()
  const templateMutation = useDownloadProductGradeTemplate()
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import Product Grades</DialogTitle>
          <DialogDescription>
            Import Product Grades from an Excel file. Download the template first to
