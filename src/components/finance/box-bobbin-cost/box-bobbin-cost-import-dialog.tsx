...
import { useImportBoxBobbinCosts, useDownloadBoxBobbinCostTemplate } from "@/hooks/finance/use-box-bobbin-cost"
import { readFileAsBytes } from "@/lib/api"

interface BoxBobbinCostImportDialogProps {
  open: boolean
...
export function BoxBobbinCostImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: BoxBobbinCostImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportBoxBobbinCosts()
  const templateMutation = useDownloadBoxBobbinCostTemplate()
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportResult({
        ...
        updatedCount: (response as { updatedCount?: number }).updatedCount ?? 0,
        failedCount: response.failedCount,
        errors: response.errors,
      })

      if (response.successCount > 0 || ((response as { updatedCount?: number }).updatedCount ?? 0) > 0) {
        onSuccess?.()
      }
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import Box/Bobbin Costs</DialogTitle>
          <DialogDescription>
            Import Box/Bobbin Costs from an Excel file. Download the template first to
