...
interface ParameterImportDialogProps {
  open: boolean
...
export function ParameterImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: ParameterImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportParameters()
  const templateMutation = useDownloadParameterTemplate()
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Import parameters from an Excel file. Download the template first to
