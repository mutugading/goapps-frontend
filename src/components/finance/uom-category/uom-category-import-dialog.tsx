...
import { useImportUOMCategories, useDownloadUOMCategoryTemplate } from "@/hooks/finance/use-uom-category"
import { readFileAsBytes } from "@/lib/api"

interface UOMCategoryImportDialogProps {
  open: boolean
...
export function UOMCategoryImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: UOMCategoryImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportUOMCategories()
  const templateMutation = useDownloadUOMCategoryTemplate()
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import UOM Categories</DialogTitle>
          <DialogDescription>
            Import UOM Categories from an Excel file. Download the template first to
