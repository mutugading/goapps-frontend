...
import { useImportInterminglings, useDownloadInterminglingTemplate } from "@/hooks/finance/use-intermingling"
import { readFileAsBytes } from "@/lib/api"

interface InterminglingImportDialogProps {
  open: boolean
...
export function InterminglingImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: InterminglingImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportInterminglings()
  const templateMutation = useDownloadInterminglingTemplate()
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import Interminglings</DialogTitle>
          <DialogDescription>
            Import Interminglings from an Excel file. Download the template first to
