...
import { ActiveFilter } from "@/types/iam/company"
...
export function CompanyCombobox({
    ...
    const displayLabel = value
        ? selectedLabel || initialLabel || "Selected company"
        : placeholder
                    ...
                    <div className="max-h-72 overflow-y-auto p-1">
                        ...
                        {!list.isFetching && items.length === 0 && (
                            <p className="px-3 py-6 text-center text-sm text-muted-foreground">
                                {debounced ? "No matching companies" : "No companies available"}
                            </p>
                        )}
                        {!list.isFetching &&
                            items.map((item) => {
                                const id = item.companyId
                                const label = `${item.companyCode} — ${item.companyName}`
