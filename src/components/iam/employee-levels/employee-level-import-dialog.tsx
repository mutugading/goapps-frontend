...
import { useImportEmployeeLevels, useDownloadEmployeeLevelTemplate } from "@/hooks/iam/use-employee-level"
import { readFileAsBytes } from "@/lib/api"

type DuplicateAction = "skip" | "update" | "error"
...
interface EmployeeLevelImportDialogProps {
  open: boolean
...
export function EmployeeLevelImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: EmployeeLevelImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportEmployeeLevels()
  const templateMutation = useDownloadEmployeeLevelTemplate()
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import Employee Levels</DialogTitle>
          <DialogDescription>
            Import employee levels from an Excel file. Download the template first to
