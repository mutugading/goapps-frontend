...
import { useImportEmployeeGroups, useDownloadEmployeeGroupTemplate } from "@/hooks/iam/use-employee-group"
...
interface EmployeeGroupImportDialogProps {
  open: boolean
...
export function EmployeeGroupImportDialog({
  open,
  onOpenChange,
  onSuccess,
}: EmployeeGroupImportDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [duplicateAction, setDuplicateAction] = useState<DuplicateAction>("skip")
  const [importResult, setImportResult] = useState<ImportResult | null>(null)

  const importMutation = useImportEmployeeGroups()
  const templateMutation = useDownloadEmployeeGroupTemplate()
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>Import Employee Groups</DialogTitle>
          <DialogDescription>
            Import employee groups from an Excel file. Download the template first to
